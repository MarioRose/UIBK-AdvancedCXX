...
class Vector : public Attributes<N> {
  public:
	...
	template <std::size_t numberOfElements = N, typename = std::enable_if_t<(numberOfElements > 3)>, typename... Args>
	Vector(Args... args) : data{double(args)...}
	{
		static_assert(sizeof...(Args) == N, "wrong number of arguments");
	}

	template <std::size_t numberOfElements = N, typename = std::enable_if_t<(numberOfElements == 1)>>
	Vector(double arg1) : data{arg1}
	{
		this->x = this->data[0];
	}
